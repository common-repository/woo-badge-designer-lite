...
if ( ! class_exists( 'WOBD_Setup' ) ) {
    ...
    class WOBD_Setup extends WOBD_Library{
        ...
        function __construct(){
            ...
            if (isset($wobd_common_settings['wobd_page_configure_option']) && $wobd_common_settings['wobd_page_configure_option'] == 'method_one') {
                add_action('woocommerce_product_thumbnails', array($this, 'wobd_display_single_page_badges'), 20);
                add_filter('woocommerce_sale_flash', array($this, 'wobd_remove_sale_text'), 2, 2);
            } else if (isset($wobd_common_settings['wobd_page_configure_option']) && $wobd_common_settings['wobd_page_configure_option'] == 'method_two') {
                add_action('woocommerce_before_single_product_summary', array($this, 'wobd_display_single_page_badges'), 20);
                add_filter('woocommerce_sale_flash', array($this, 'wobd_remove_sale_text'), 2, 2);
            } else {
                add_filter('woocommerce_sale_flash', array($this, 'wobd_custom_sale_text'), 10, 2);
            }
            if (isset($wobd_common_settings['wobd_shop_configure_option']) && $wobd_common_settings['wobd_shop_configure_option'] == 'method_one') {
                add_filter('post_thumbnail_html', array($this, 'wobd_badge_on_product'), 999, 1);
                add_filter('woocommerce_product_get_image', array($this, 'wobd_badge_on_product'), 999, 1);
                add_filter('woocommerce_sale_flash', array($this, 'wobd_remove_sale_text'), 2, 2);
            } else if (isset($wobd_common_settings['wobd_shop_configure_option']) && $wobd_common_settings['wobd_shop_configure_option'] == 'method_two') {
                add_action('woocommerce_before_shop_loop_item_title', array($this, 'wobd_badge_wrapper_start'), 5, 2);
                add_action('woocommerce_before_shop_loop_item_title', array($this, 'wobd_display_product_badges_action'));
                add_action('woocommerce_before_shop_loop_item_title', array($this, 'wobd_badge_wrapper_close'), 12, 2);
                add_filter('woocommerce_sale_flash', array($this, 'wobd_remove_sale_text'), 2, 2);
            } else {
                add_filter('woocommerce_sale_flash', array($this, 'wobd_custom_sale_text'), 10, 3);
            }
        }
        ...
        function wobd_individual_badge_settings(){
            add_meta_box( 'wobd_each_badges_settings', __( 'Badge Designer Lite For WooCommerce Setting', WOBD_TD ), array( $this, 'wobd_each_badge_action' ), 'product', 'advanced', 'low' );
        }
        ...
        function wobd_display_single_page_badges() {
            ...
            if (isset($wobd_common_settings['wobd_enable_single_page_badge']) && $wobd_common_settings['wobd_enable_single_page_badge'] == '1') {
                ...
                if ($wobd_common_settings['wobd_default_sale_badge'] != 'disable' && $wobd_common_settings['wobd_default_sale_badge'] != 'default') {
                    if ($product->is_on_sale() && $product->is_in_stock()) {
                        $wobd_post_id = $wobd_common_settings['wobd_default_sale_badge'];
                        $wobd_option = get_post_meta($wobd_post_id, 'wobd_option', true);
                        include(WOBD_PATH . 'includes/frontend/template/sales/sales-content.php');
                    }
                }
...
// save data
        ...
        function wobd_custom_sale_text($text, $flash) {
            ...
            if (is_product()) {
                if ((isset($wobd_common_settings['wobd_default_sale_badge'])) && ($wobd_common_settings['wobd_default_sale_badge'] == 'disable')) {
                    $text = '';
                    return $text;
                } else if ( (isset($wobd_common_settings['wobd_default_sale_badge'])) && ($wobd_common_settings['wobd_default_sale_badge'] == 'default')) {
                    return $text;
                } else {
                    ob_start();
                    if (isset($wobd_common_settings['wobd_enable_single_page_badge']) && $wobd_common_settings['wobd_enable_single_page_badge'] == '1') {
                    ...
                    $text = '<div class="wobd-badges-wrapper">' . $data . '</div>';
                    ob_end_clean();
                    return $text;
                }
            } else {
                if ((isset($wobd_common_settings['wobd_default_sale_badge'])) && ($wobd_common_settings['wobd_default_sale_badge'] == 'disable')) {
                    ...
                    if (is_admin()) {
                        ...
                        if (is_cart() || is_checkout() || is_account_page() || is_wc_endpoint_url()) {
                            ...
                            $html = '<div class="wobd-badges-wrapper">' . $data . '</div>';
                        }
                    }
                    ob_end_clean();
                }
        ...
        function wobd_badge_wrapper_start() {

            echo '<div class="wobd-badges-wrapper">';
        }

        // Close the div that we just added
        function wobd_badge_wrapper_close() {
            echo '</div>';
        }
        ...
        function wobd_badge_on_product($html) {
            ...
            global $product;
            $wobd_common_settings = get_option('wobd_common_settings');
            $wobd_low_stock_qty = isset( $wobd_common_settings['wobd_low_stock_qty'] ) ? esc_attr( $wobd_common_settings['wobd_low_stock_qty'] ) : '3';
            if ($product) {
                    ob_start();
                    $wobd_common_settings = get_option('wobd_common_settings');
                    include(WOBD_PATH . 'includes/frontend/badges.php');
        ...
        function wobd_display_product_badges_action() {
            global $product;
            $wobd_common_settings = get_option('wobd_common_settings');
            include(WOBD_PATH . 'includes/frontend/badges.php');
            if ($product->is_on_sale() && $product->is_in_stock()) {
        ...
        function wobd_remove_sale_text($text, $flash) {
            $wobd_common_settings = get_option('wobd_common_settings');
